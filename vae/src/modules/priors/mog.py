...
class MixtureOfGaussians(Prior):
    """
    Reposetory: https://github.com/divymurli/VAEs

    Args:
        z_dim: int: Number of latent dimensions
        k: int: Number mixture components in MoG prior
    """
    def __init__(self, z_dim, num_mixtures=10):
        super().__init__()
        self.z_dim = z_dim
        self.k = num_mixtures
    ...
    def sample_gaussian(self, m, v):
        """ Element-wise application reparameterization trick 
            to sample from Gaussian.
        """
        epsilon = torch.randn(m.shape).to(args.device)
        z = m + (v**0.5)*epsilon
        return z

    def log_sum_exp(self, x, dim=0):
        """ Compute the log(sum(exp(x), dim)) in a numerically
            stable manner.
        """
    ...
    def log_mean_exp(self, x, dim):
        """ Compute the log(mean(exp(x), dim)) 
            in a numerically stable manner.
        """
        return self.log_sum_exp(x, dim) - np.log(x.size(dim))

    def log_normal(self, x, m, v):
        """ Computes the elem-wise log probability of a Gaussian and
            then sum over the last dim. Basically we're assuming all
            dims are batch dims except for the last dim.        
        """
        const   = -0.5 * self.z_dim * torch.log(2*torch.tensor(np.pi))
    ...
    def gaussian_parameters(self, h, dim=-1):
        """ Converts generic real-valued representations into mean
            and variance parameters of a Gaussian distribution.
        """
    ...
    def sample(self, n_samples=1, **kwargs):
        ...
        m, v = m[idx], v[idx]
        return self.sample_gaussian(m, v)
